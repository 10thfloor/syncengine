...
import type { TableDef, ViewDef } from './schema';
import type { Migration } from './migrations';
import { extractMergeConfig } from './schema';
import { tableToCreateSQL, tableToInsertSQL } from './sql-gen';
import { CONFLICT_LOG_MAX } from './constants';
...
export type ConnectionStatus = 'off' | 'connecting' | 'syncing' | 'connected' | 'disconnected' | 'auth_failed';
...
export interface SyncConfig {
    workspaceId: string;
    natsUrl?: string;     // default: ws://localhost:9222
    restateUrl?: string;  // default: http://localhost:8080
    authToken?: string;   // JWT for NATS + Restate auth
    userId?: string;      // user identity for message attribution
}
