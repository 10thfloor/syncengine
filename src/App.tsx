...
import { table, id, real, text, view, store, sum, count, avg, max } from './lib';
import type { ConflictRecord, SyncStatus, ConnectionStatus, Migration } from './lib';
import './App.css';
...
const topExpenses = view('topExpenses', expenses)
    .topN('amount', 5, 'desc');
...
const totals = view('totals', expenses)
    .aggregate([], {
        total: sum('amount'),
        count: count(),
        avg: avg('amount'),
...
const EMOJI: Record<string, string> = {
    Food: '🍔', Travel: '✈️', Software: '💻', Office: '📦', Entertainment: '🎬',
};
const BUDGET_LIMITS: Record<string, number> = {
    Food: 2000, Travel: 5000, Software: 3000, Office: 4000, Entertainment: 1500,
};
...
function SyncBadge({ status, syncStatus }: { status: ConnectionStatus; syncStatus: SyncStatus }) {
    ...
    if (status === 'syncing' || syncStatus.phase === 'replaying') {
        const pct = syncStatus.totalMessages
            ? Math.round((syncStatus.messagesReplayed / syncStatus.totalMessages) * 100)
            : null;
        return (
...
function ConflictPanel({ conflicts, onDismiss }: { conflicts: ConflictRecord[]; onDismiss: (i: number) => void }) {
        ...
        <div className="panel conflict-panel">
            ...
            <div className="conflict-list">
                {active.map((c, i) => {
                    ...
                    return (
                        <div key={`${c.table}-${c.recordId}-${c.field}-${c.resolvedAt}`} className="conflict-item">
                            <div className="conflict-header">
                                <span className="conflict-field">{c.table}.{c.field}</span>
                                <span className="badge purple" style={{ fontSize: '10px' }}>{c.strategy.toUpperCase()}</span>
                                <button className="conflict-dismiss" onClick={() => onDismiss(realIndex)}>dismiss</button>
                            </div>
                            <div className="conflict-values">
                                <span className="conflict-winner">Winner: {JSON.stringify(c.winner.value)}</span>
                                <span className="conflict-loser">Loser: {JSON.stringify(c.loser.value)}</span>
                            </div>
                        </div>
                    );
                })}
...
export default function App() {
    ...
    const seedBudgets = useCallback(() => {
        CATEGORIES.forEach((cat, i) => {
            db.insertSeed('budgets', { id: i + 1, category: cat, budget: BUDGET_LIMITS[cat] });
        });
    }, []);
    useEffect(() => { if (ready) seedBudgets(); }, [ready, seedBudgets]);
    useEffect(() => { if (ready && syncStatus.phase === 'live') seedBudgets(); }, [syncStatus.phase, ready, seedBudgets]);

    // Cmd+Z / Ctrl+Z undo
    const handleUndo = useCallback(() => { db.undo(); }, []);
    useEffect(() => {
    ...
    return (
        <div className="app">
            ...
            <div className="header">
                ...
                <div className="header-badges">
                    ...
                    {activeConflicts.length > 0 && (
                        <span className="badge orange">{activeConflicts.length} conflict{activeConflicts.length !== 1 ? 's' : ''}</span>
                    )}
                </div>
            </div>
            ...
            <div className="add-bar">
                ...
                <button className="btn btn-primary" onClick={handleAdd} disabled={!ready}>
                    Add
                </button>
                <button className="btn btn-ghost" onClick={handleRandom} disabled={!ready}>
                    Random
                </button>
                <button className="btn btn-ghost btn-sm" onClick={handleUndo} disabled={undoSize === 0}>
            ...
            {/* ── Conflict panel ──────────────────────────────────────────── */}
            <ConflictPanel conflicts={conflicts} onDismiss={(i) => db.dismissConflict(i)} />
            ...
            <div className="panels">
                            ...
                            {[...categories]
                                ...
                                .map((cat, i) => (
                                    <div key={`${cat.category}-${i}`} className="cat-chip">
                                        <span className="cat-emoji">{EMOJI[cat.category as string] ?? '📁'}</span>
                                        <span className="cat-name">{cat.category as string}</span>
                                        <span className="cat-amount">{fmt(cat.total as number)}</span>
                                    </div>
                                ))}
                        </div>
                    </div>
                ...
                <div className="panels-right">
                    <div className="panel">
                        <h2>Budget Tracker <span className="badge purple" style={{ fontSize: '0.65rem', verticalAlign: 'middle' }}>JOIN</span></h2>
                                    ...
                                    return (
                                        <li key={`${row.category}-${i}`} className="budget-item">
                                            <div className="budget-header">
                                                <span className="name">
                                                    {EMOJI[row.category as string] ?? '📁'} {row.category as string}
                                                </span>
                ...
                <div className="debug-panel">
                    <div className="debug-grid">
                        <div className="debug-item">
                            ...
                            <span className={`debug-value ${netStatus === 'connected' ? 'ok' : 'warn'}`}>{netStatus}</span>
                        </div>
                        <div className="debug-item">
                        ...
                        <div className="debug-item">
                            ...
                            <span className="debug-value">{syncStatus.messagesReplayed}{syncStatus.totalMessages ? ` / ${syncStatus.totalMessages}` : ''}</span>
                        </div>
                        <div className="debug-item">
                            <span className="debug-label">Snapshot</span>
                            <span className="debug-value">{syncStatus.snapshotLoaded ? 'loaded' : 'none'}</span>
                        </div>
                        <div className="debug-item">
                            <span className="debug-label">Conflicts</span>
                            <span className="debug-value">{conflicts.length} total, {activeConflicts.length} active</span>
                        </div>
                        <div className="debug-item">
                    ...
                    <div className="debug-actions">
                        <button className="btn btn-ghost btn-sm" onClick={handleSimulateConflict} disabled={top.length === 0}>
                            Simulate conflict
                        </button>
                    </div>
                </div>
